...
```{r}
#' @concept This is creating a nested data frame. The main difference between a nested data frame 
#and grouped data frame is that for a grouped data frame, each row is an observation.
#For a nested data frame each row is a group of observations!
...
```{r growth categorization}
#this is for categorizing Liz's variants because some of them didn't grow!
#If they had a .5OD by the end of the experiment I didn't even try to fit the model to them because it would break!

for (i in 1:nrow(by_variant)) {
  if (max(by_variant$data[[i]][[2]]) > .75 ){
    by_variant$Growth[i] <- "Growth"
  } else if (max(by_variant$data[[i]][[2]]) <= .75 & max(by_variant$data[[i]][[2]]) > .5) {
    by_variant$Growth[i] <- "Inhibited"
  } else {
    by_variant$Growth[i] <- "Minimal"
  }
}
```
...
p <- readydata %>%
  filter(Plate == 1 & Inducer == "4-HT") %>%
  ggplot(aes(x = time, y = OD600, color = Plasmid)) +
  geom_point() +
...
#looks like everything besides negative and DV41 looks like it may be increasing over time.
#Also looks like the positive control pLW002 has peak OD at 17 hours. Therefore, I will cut all values off for other data.
```
```{r, fig.height = 6, fig.width = 8}
w <-readydata %>%
  filter(Plate == 2 & Inducer == "4-HT") %>%
  ggplot(aes(x = time, y = OD600, color = Plasmid)) +
  geom_point() +
  labs(title = "4-HT condition") +
  ggthemes::scale_color_colorblind() +
...
ggarrange(w,z, common.legend = TRUE)
...
```{r}
#This loop finds the max OD for each column. Then based off it's index removes all rows an hour past the maximum. This is to ensure that the maximum has actually occurred and the stationary phase of growth is evident for the model.

for (i in 1:nrow(by_variant)) {
  holder <- by_variant$data[[i]]
  
  max_OD <- which.max(holder$OD600)
  
  holder[1:(max_OD + 6),] -> by_variant$data[[i]]
