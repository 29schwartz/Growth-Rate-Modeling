...
```{r}
#This code will take the growth rate of the sample in the incomplete media and calculate it as a percent of growth of that of the cells in the complete media!

#With the filter function, you want to end up filtering for the complete media!

fitted_variants %>%
  group_by(Plasmid, Replicate) %>%
  mutate(difference = (estimate_mu - lag(estimate_mu, default = last(estimate_mu))) / estimate_mu) %>%
  filter(Media == "M9C") -> mu_percent
```

```{r}
#plotting this calculated percent!
mu_percent %>%
  ggplot(aes(x = Plasmid, y = estimate_mu)) + 
