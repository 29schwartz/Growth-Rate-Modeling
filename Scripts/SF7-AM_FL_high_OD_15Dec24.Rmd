...
```{r}
nested_data %<>%
  filter(grepl('SKS015|SKS024|SKS025', Plasmid)) %>%
  #applying custom function to find slopes
  mutate(slope = map(data, Slope_Determining)) %>%
  #fitting linear model
  mutate(model = map(slope, function(df) lm(H2S_uM_start ~ start_time, data = df))) %>%
